  ...
  if (m_state == LAST_ACK)
    {
     ...
     Time lastRto = m_rtt->GetEstimate () + m_rtt->GetVariation () * (1/calculateGain()), m + 2 * m_clockGranularity;
...
TcpSocketBase::SendEmptyPacket (uint8_t flags)
{
  ...
  m_rto = Max (m_rtt->GetEstimate () + m_rtt->GetVariation () * (1/calculateGain()), m + 2 * m_clockGranularity);
...
double TcpSocketBase::calculateSamplingRate(){
  //uint32_t nsamples = m_rtt->GetNSamples();
  return 2;
  //  tcpPayloadSize
  //  m_txBuffers
  //  headsequence() - tailsequence()
  // unacknowledged data count function

}

double TcpSocketBase::calculateGain( ){


  return 1/3;
  
  // uint32_t rttSamplingRate = calculateSamplingRate();
  // //we need to calculate sampling rate
  // if (rttSamplingRate == 1){
  //   return (1/BytesInFlight());
  // }

  // else if(rttSamplingRate == 1/2){
  //   return (2/BytesInFlight());
  // }

  // else if(rttSamplingRate == 2){
  //   return 1/3;
  // }
...
TcpSocketBase::EstimateRtt (const TcpHeader& tcpHeader)
{
  ...
  if (!m.IsZero ())
    {
      // m_rtt->Measurement (m,calculateGain());                // Log the measurement
      m_rtt->Measurement (m,calculateGain());                // Log the measurement
      // RFC 6298, clause 2.4
      // m_rto = Max (m_rtt->GetEstimate () + Max (m_clockGranularity, m_rtt->GetVariation () * 4), m_minRto);
      m_rto = Max (m_rtt->GetEstimate () + m_rtt->GetVariation () * (1/calculateGain()), m + 2 * m_clockGranularity);
